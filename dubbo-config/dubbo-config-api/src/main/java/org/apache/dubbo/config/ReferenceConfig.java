...
public class ReferenceConfig<T> extends ReferenceConfigBase<T> {
    ...
    /**
     * 入口，会被 ReferenceAnnotationBeanPostProcessor 调用
     * @return
     */
...
//                    consumerModel.getMethodModel(methodConfig.getName()).addAttribute(ASYNC_KEY, asyncMethodInfo);
    ...
    private T createProxy(Map<String, String> map) {
        if (shouldJvmRefer(map)) { // 判断是否为Injvm调用
            URL url = new URL(LOCAL_PROTOCOL, LOCALHOST_VALUE, 0, interfaceClass.getName()).addParameters(map);
            // 生成对应的invoker，这里面可能还被包装了filter
                ...
                if (!LOCAL_PROTOCOL.equalsIgnoreCase(getProtocol())) {
                    ...
                    if (CollectionUtils.isNotEmpty(us)) {
                        for (URL u : us) {
                            ...
                            if (monitorUrl != null) {
                                // 注册到监控中心，类似与register一样，再套一层
                                map.put(MONITOR_KEY, URL.encode(monitorUrl.toFullString()));
                            }
                            // 注册中心地址后添加refer存储服务消费元数据信息
                            // 应用启动时订阅注册中心，服务提供者参数等合并是会用到这部分信息。
        ...
        // create service proxy 默认是JDK动态代理
