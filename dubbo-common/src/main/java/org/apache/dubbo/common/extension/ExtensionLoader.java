...
public class ExtensionLoader<T> {
    ...
    /**
     * 获取dubbo-common模块下META—INF中LoadingStrategy的所有类，
     * 当前共有三个
    ...
    private ExtensionLoader(Class<?> type) {
        ...
        null :
        // 获取ExtensionLoader的自适应实现，然后用它来创建对应类的ExtensionLoader实例
     ...
     * 创建对应当前接口的实现
     * @param name 根据名称进行实例
     * @param wrap 是否包装 一般都是true
     * @return
     */
    @SuppressWarnings("unchecked")
    private T createExtension(String name, boolean wrap) {
        //  获取接口的所有实现类。name-class的map。
        //  然后在根据名称获取对应的实现类
        Class<?> clazz = getExtensionClasses().get(name);
        ...
        try {
            ...
            if (wrap) {
                ...
                if (CollectionUtils.isNotEmpty(wrapperClassesList)) {
                    // CORE_CODE 包装类实例，存在多次包装
                    for (Class<?> wrapperClass : wrapperClassesList) {
                        Wrapper wrapper = wrapperClass.getAnnotation(Wrapper.class);
                        if (wrapper == null
                                || (ArrayUtils.contains(wrapper.matches(), name) && !ArrayUtils.contains(wrapper.mismatches(), name))) {
                            logger.info("接口包装："+type.getSimpleName()+"\t "+wrapperClass.getSimpleName());
                            // 调用构造方法实例化，并处理依赖注入
    ...
    /**
     * 依赖注入，通过setter注入属性
     * @param instance 被注入的类
     * @return
     */
    private T injectExtension(T instance) {

        // 如果是ExtensionLoader，就直接返回
    ...
    /**
     * 记载type接口的实现类，并分类@Adaptive和wrapper实现
     * synchronized in getExtensionClasses
     */
    private Map<String, Class<?>> loadExtensionClasses() {
        // 获取SPI的value，也就是默认的拓展name，保存到cachedDefaultName
        cacheDefaultExtensionName();

        Map<String, Class<?>> extensionClasses = new HashMap<>();
        // 通过三种不同的目录获取配置的对应的接口实现
    ...
    /**
     * 解析dubbo目录下的SPI接口实现类文件，获得name-class的数据
     *
     * @param extensionClasses 用于返回name-class的数据
     * @param classLoader ExtensionLoader的类加载器
     * @param resourceURL 实现
     * @param overridden
     * @param excludedPackages 忽略某些包
     */
    ...
    /**
     * 加载class，并将@Adaptive和wrapper实现 分类到各自的缓存
     */
    private void loadClass(Map<String, Class<?>> extensionClasses, java.net.URL resourceURL, Class<?> clazz, String name,
        ...
        // CORE_CODE 处理@Adaptive和wrapper接口
        if (clazz.isAnnotationPresent(Adaptive.class)) {
    ...
    private void cacheAdaptiveClass(Class<?> clazz, boolean overridden) {
        // 如果SPI的默认参数为空, 在类上标注了Adaptive注解，那么它就是默认的实现
    ...
    private Class<?> createAdaptiveExtensionClass() {
        ...
        logger.info("生成了Adaptive类，"+type.getSimpleName()+"\t adaptive实现是："+ cachedDefaultName);
        ClassLoader classLoader = findClassLoader();
